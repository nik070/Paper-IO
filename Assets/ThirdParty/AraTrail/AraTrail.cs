...
namespace Ara{
    ...
    public class AraTrail : MonoBehaviour {
        ...
        public Transform uvDisablePosition = null;
        ...
        private void RenderMesh(Camera cam)
        {
            ...
            {
#if UNITY_LUNA
                Vector3 pos = renderMatrix.GetColumn(3);
                Quaternion rot = renderMatrix.rotation;
                Graphics.DrawMesh(mesh_, pos, rot, materials[i], gameObject.layer);
#else
        ...
        private void AppendFlatTrail(Point[] data, ref CurveFrame frame, int i, int count, float sectionThickness, float vCoord, ref int va, ref int vb)
        {
            ...
            if (hqCorners && cornerRoundness > 0)
            {
                ...
                for (int p = 0; p <= cornerRoundness; ++p)
                {
                    ...
                    if (uvDisablePosition != null)
                    {
                        if (data[i].position.z < uvDisablePosition.position.z)
                        {
                            uv.Set(vCoord, 1, 0, 0);
                        }
                    }

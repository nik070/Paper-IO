...
//                  "• 128  — fast bake, great performance.\n" +
//                  "• 256  — good quality, may slow down captures slightly.\n" +
//                  "• 512+ — extremely slow, not recommended for gameplay.")]
//         [Range(64, 1024)] [SerializeField] private int _generationResolution = 128;
...
//                  "0 = raw pixel blocks.  2 = soft curves.  3 = maximum safe value.")]
//         [Range(0, 6)] [SerializeField] private int _smoothingIterations = 2;
...
//             {
//                 // Clamp iterations to 3 max to prevent exponential vertex explosion (causes massive lag on captures)
//                 int safeIterations = Mathf.Min(_smoothingIterations, 3);
//                 unioned = ApplyChaikin(unioned, safeIterations);
//             }

//             // ALWAYS simplify paths to aggressively reduce vertex count for gameplay performance.
//             // We use an epsilon of ~0.5 world units which preserves the shape but removes thousands of redundant vertices.
//             double epsilon = Mathf.Max((rectSize.x / res) * 0.5f, 0.5f) * GeometryUtils.Scale;
